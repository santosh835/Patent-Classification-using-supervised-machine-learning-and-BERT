...
df = pd.read_csv('data/PatentData_train.csv')


#df["patentDetails"]=df["abstracts"] + df["patent_title"] 
#Explore and Preprocess
df["patentDetails"]=df["description"]
df=df[['classifications',
 'patentDetails',
]]
df = df[df['patentDetails'].notna()]
df = df[df['classifications'].notna()]

#replace to full forms of Patent classes
...
possible_labels = df.classifications.unique()
############################################################
#Encoding the Labels
label_dict = {}
...
df['label'] = df.classifications.replace(label_dict)
...
df.groupby(['classifications', 'label', 'data_type']).count()
###############################################################
...
encoded_data_train = tokenizer.batch_encode_plus(
    df[df.data_type=='train'].patentDetails.values, 
...
encoded_data_val = tokenizer.batch_encode_plus(
    df[df.data_type=='val'].patentDetails.values, 
    add_special_tokens=True, 
    return_attention_mask=True, 
    pad_to_max_length=True, 
    max_length=256, 
